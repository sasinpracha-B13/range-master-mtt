...
- **Last committed + pushed**: `v4.1.2` — Module 2 Architecture + Data Plan (`c6a24ac`).
- **Pending push (STAGED)**: `v4.1.3` — Module 2 Audit Tooling. New `tools/audit-postflop-module2-seed.ps1` (~620 LOC) implements 30 hard rules + 7 soft warnings + 9 coverage axes for Module 2 seed scenarios. Includes suit-count discipline, hand-class mechanical derivation (suit counts, made flush/draw, straight, OESD, gutshot, pair classes, kicker analysis), explanation completeness, schema vocabulary, sourceConfidence honesty, mojibake detection (Thai-range + replacement-char codepoint scan from ASCII-only source), and strategic soft warnings (overpair-on-low-connected, naked-air-on-monotone, etc.). Reads with explicit UTF-8 to avoid system-codepage mis-decoding (CP874 false positives on Thai-locale dev machines). Result: **0 hard errors, 11 warnings, 15 PASS / 9 WARN / 0 FAIL**. Production audit `tools/audit-postflop-ps.ps1` deliberately untouched (would fail the 11 baseline Module 2 scenarios). 3 vocabulary fixes applied to seed (`underpair` → `mid_pair` in 3 obvious cases caught by audit). Production code, service worker, scenarios JSON, and gamification all untouched. New `docs/specs/postflop-v4.1.3-module2-audit-tooling-report.md` documents implementation, findings, limitations, and next steps.
