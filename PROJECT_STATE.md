...
- **Last committed + pushed**: `v4.1.3` — Module 2 Audit Tooling (`eafdf6d`).
- **Pending push (STAGED, planning-only)**: `v4.1.4` — Module 2 Seed GPT Review + Baseline Migration Decision. Two new docs in `docs/specs/`: (a) `postflop-v4.1.4-module2-seed-review-report.md` — strategic re-review of all 24 seeds with PASS/WARN/FAIL verdict (**20 PASS / 4 WARN / 0 FAIL**), per-warning disposition, recommended seed cleanup (3 `backdoor_only` relabels + 1 reword), v4.1.5 scope; (b) `postflop-v4.1.4-module2-baseline-migration-plan.md` — chose **Option C (Refactor/Migrate)** for the 11 legacy Module 2 baseline scenarios with full mapping (`bet_33 → bet_small`, `bet_75 → bet_big`, derive heroHandRole / drawCategory / showdownValue / actionReason / takeaway), audit gates (target production 286/0/0 after merge), GPT review gates, rollback plan. Plus an appended **v4.1.4 Review Addendum** in `postflop-v4.1.2-module2-gpt-review-package.md`. Production code, service worker, scenarios JSON, audit script, concepts JSON, taxonomy JSON, runtime files, and gamification all untouched. Production audit still 262/0/0. Module 2 seed audit still 24/0 hard errors / 11 warnings (15 PASS / 9 WARN / 0 FAIL). Module 2 NOT productionized; runtime UI NOT touched.
