...
- **Last committed + pushed**: `v4.1.1` — Postflop Concept Library Drill Actions (`cf088d1`).
- **Pending push (STAGED, planning-only)**: `v4.1.2` — Module 2 Architecture + Data Plan (planning-only sprint, **post-fix-pass v0.1.1**). Five new docs in `docs/specs/`: architecture, schema/taxonomy (additive — adds `heroHandRole`, `drawCategory`, `showdownValue`, `recommendedAction`, `actionReason`, `blockerNote`; vocabulary extended in fix-pass with `straight`, `flush`, `nut_flush`), 24 hand-authored seed scenarios (6 board buckets × 4 hands; 18 action_choice + 6 reason_choice; auditStatus=review_pending; sourceConfidence=expert_judgment), audit plan (30 hard rules + 8 soft rules + 5 coverage rules + suit-count discipline), and GPT review package (post-fix-pass: PASS/WARN/FAIL classification 21/3/0, 10 high-risk scenarios with 4 RESOLVED/2 partial/4 still-high, 13 reviewer questions). **Fix-pass corrected 5 mechanical errors** (scenarios #11 combo_draw→straight, #13 NFD→backdoor_only with action shift, #21 set→nut_flush_draw, #22 explanation rewrite + action shift to mixed, #24 explanation rewrite + reason shift to give_up) **and applied 5 labelling improvements** (#2 backdoor_only, #5 top_pair_good_kicker, #6 gutshot, #17 check tier upgrade, #18 actionReason swap). Production code, service worker, scenarios JSON, audit script, and gamification all untouched. Existing audit still 262/0/0. State files updated only. **NO COMMIT** until human review of the planning package.
- **Service worker `VERSION`**: `'v4.1.1'`.
- **App backup `appVersion`**: `'4.1.1'`.
