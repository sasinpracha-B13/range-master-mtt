...
- **Last committed + pushed**: `v4.1.8` — Home Mode Tabs + Module 2 Mastery + Concept-Pool Depth Audit (`5eb12ac`).
- **Pending push (STAGED, M2 DATA EXPANSION)**: `v4.1.9` — Module 2 Data Expansion + Tester Pass. **Module 2 grows from 35 to 49 production scenarios.** Production audit gate raised from 286/0/0 to **300/0/0** (251 M1 + 49 M2). Authored 14 new M2 scenarios on 14 new boards targeting the v4.1.8 primary-tag depth gaps: blocker_pressure +4 (B1 AhTc on Kh9h4c, B2 AhJh on KcKd4c, B3 AdKs on QhJh8c, B4 KhQc on AhAd8s), value_betting +3 (V1 KsQs on Kc9d4h, V2 TcTd on Th6s2c, V3 KhQh on KsQc8d), range_advantage_stab +3 (R1 8c7c on As9c3d, R2 KsQs on AsTd5c, R3 JdTh on Kc9c4d), pot_control +2 (P1 8c8s on KhTd4s, P2 JhJc on QdTh6s), give_up_strategy +2 (G1 4h3h on JcTc8c, G2 7d6d on JsTc9c). All 14 use action_choice qtype. Authored with auditStatus=review_pending; production audit caught 1 textureTag typo (`semi_wet → wet` on B3 + P2) which was fixed; final GPT/strategic review in `postflop-v4.1.9-final-gpt-review-of-14-new.md` returned 14/14 PASS; flipped auditStatus → approved + reviewStatus → v4.1.9_gpt_reviewed. **Per-concept primary-tag depth target hit**: value_betting 5→8, pot_control 6→8, blocker_pressure 4→8, give_up_strategy 6→8, range_advantage_stab 5→11 (over-target due to cross-tagging on B1/B2/B4). Runtime correctly loads all 300 scenarios. Browser QA: 30/30 PASS (M2 from curriculum + 5 M2 concept drills + M1 normal + M1 concept + preflop + mobile + console). **appVersion 4.1.8 → 4.1.9, SW VERSION v4.1.8 → v4.1.9.** No schema changes, no taxonomy changes, no audit-script changes, no preflop touched. Two new docs in `docs/specs/`: `postflop-v4.1.9-data-expansion-tester.md` (30-check QA + per-concept depth verification) and `postflop-v4.1.9-final-gpt-review-of-14-new.md` (14/14 PASS strategic review).

- **Service worker `VERSION`**: `'v4.1.9'`.
- **App backup `appVersion`**: `'4.1.9'`.
