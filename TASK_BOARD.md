...
🟡 **v4.1.3 staged — Module 2 Audit Tooling.** New `tools/audit-postflop-module2-seed.ps1` (~620 LOC, ASCII-only source) implements 30 hard rules + 7 soft warnings + 9 coverage axes from `audit-plan.md`. Suit-count discipline + mechanical hand-class derivation would have caught the v4.1.2 mechanical errors automatically. Reads JSON with explicit UTF-8 (avoids CP874 phantom-mojibake on Thai-locale Windows). Production auditor `tools/audit-postflop-ps.ps1` deliberately untouched (would fail the 11 baseline Module 2 scenarios). Result: 0 hard errors, 11 warnings, 15 PASS / 9 WARN / 0 FAIL. Applied 3 vocabulary refinements to seed (`underpair` → `mid_pair`). Production runtime untouched (audit still 262/0/0; no app version bump). Awaiting commit.
...
- 2026-05-05: v4.1.3 Module 2 Audit Tooling STAGED. New seed auditor catches all v4.1.2 mechanical-error categories. 0 hard errors / 11 documented warnings / 15 PASS / 9 WARN / 0 FAIL. Production audit still 262/0/0. Awaiting commit.
