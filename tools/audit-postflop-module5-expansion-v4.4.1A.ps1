# ============================================================
# audit-postflop-module5-expansion-v4.4.1A.ps1
# v4.4.1A -- Module 5 EXPANSION seed auditor.
# Validates docs/specs/postflop-v4.4.1A-module5-expansion-seeds.json
...
if (-not $SeedPath) { $SeedPath = Join-Path $repoRoot 'docs\specs\postflop-v4.4.1A-module5-expansion-seeds.json' }
...
# Batch-level
if (-not ([string]$j.version).StartsWith('v4.4.1A')) { Add-Hard 'X01' 'TOP' "version should start v4.4.1A, got '$($j.version)'" }
$scn = @($j.scenarios)
...
foreach ($s in $scn) {
  ...
  if ($s.reviewStatus -ne 'v4.4.1A_expansion_candidate') { Add-Hard 'X02' $sid "reviewStatus expected v4.4.1A_expansion_candidate, got '$($s.reviewStatus)'" }
...
foreach ($d in $dups) { Add-Hard 'XDUP' $d.Name "duplicate id within v4.4.1A batch (x$($d.Count))" }
try {
...
Write-Host ''
Write-Host '================================================================'
Write-Host ' M5 EXPANSION SEED AUDIT -- v4.4.1A (planning_only)'
Write-Host '================================================================'
