...
# 1.2 -- 76 mid pair (no draw) on brick A-high - bluff catch
$scenarios += New-Scenario `
  -id 'pf_btn_v_bb_srp_100bb_turn_Ac7d2s_4h_m4_action_7s6h_v430C' `
  -board $f1 -heroHand @('7s','6h') `
  -handClass 'mid_pair' -heroHandRole 'bluff_catcher' -drawCategory 'none' -showdownValue 'decent' `
  -blockerNote 'No straight draw: hero+board ranks 2,4,6,7,A leave a 2-card gap (3 and 5 both needed) for 3-4-5-6-7. One-card gutshot does NOT exist.' `
  ...
  -explanation (New-Explanation `
    'Middle pair 7 with no draw on brick turn -- call as bluff-catcher vs polarized barrel.' `
    '4h is a brick. Hero+board ranks are 2,4,6,7,A: no one-card straight is available (3-4-5-6-7 needs both 3 and 5; 4-5-6-7-8 needs both 5 and 8). Hero has middle pair 7 with no draw equity, only showdown value vs villain bluffs.' `
    'BB flop call with 7-6o on A-7-2 was natural (middle pair). Turn 4 brick keeps hero at middle pair with no draw added.' `
    '7s6h has middle pair (7) with 6 kicker. Beats villain air and worse pairs (66, 55, 33, 22 if any). Loses to Ax, 77, sets, two-pair. Calling captures villain bluff frequency.' `
    'Calling realizes pair showdown value vs polarized barrel. Raising thin pair OOP folds out bluffs and isolates against value.' `
    'Folding middle pair vs small barrel under-defends the BB calling range and pays off villain over-bluffing.' `
    'Middle pair no draw on brick turn = call (bluff-catch).') `
  -conceptTags @('turn_bluff_catcher','second_barrel_defense','turn_range_disadvantage') `
  -difficulty 3 `
  -uniquenessNote 'Pure bluff-catch call with naked middle pair (no draw) on A-high brick. Distinct from family 1 OESD (1.1) which has 8-out draw equity, and from underpair 99 (1.3) which mixes -- here hero has a clean middle-pair-only call for showdown value.'
...
# 8.1 -- 9d6d T-high straight on draw-intensifier turn - value raise
$scenarios += New-Scenario `
  -id 'pf_btn_v_bb_srp_100bb_turn_Ts8s4d_7c_m4_action_9d6d_v430C' `
  -board $f8 -heroHand @('9d','6d') `
  -handClass 'straight' -heroHandRole 'nutted_value' -drawCategory 'none' -showdownValue 'nutted' `
  -blockerNote 'Hero made T-high straight 6-7-8-9-T. The only higher straight on this turn is J-high (7-8-9-T-J), which requires J9 in villain (rare combo given hero blocks one 9).' `
  -recommendedAction 'check_raise_small' -actionReason 'value_check_raise_turn' `
  ...
  -explanation (New-Explanation `
    ...
    '7c completes hero straight: hero+board ranks 4,6,7,8,9,T = 6-7-8-9-T = T-high straight. The only higher straight is J-high (7-8-9-T-J) which requires villain to hold J9 (hero blocks one of the four 9s, leaving 9 J9 combos). Sets (TT/88/44), two-pair, and Tx are all behind hero.' `
    'BB flop call with 96-suited on T-8-4 was thin (overcards + BDFD); turn 7 lands the straight.' `
    '9d6d makes 6-7-8-9-T = T-high straight. Loses only to J9 (J-high straight). Beats Tx/overpairs/sets/two-pair/draws.' `
    'Small check-raise charges Tx, overpairs, sets, and weaker draws. Big raise on a dynamic turn also justified vs sets that may continue.' `
    'Slowplaying second-nut straight on dynamic turn surrenders value as flush/higher-straight rivers kill action.' `
    'Made T-high straight on draw-intensifier turn = check-raise for value.') `
...
$out = [PSCustomObject]([ordered]@{
  ...
  expansionStats   = [PSCustomObject]@{
    ...
    finalProductionTarget   = '53 (24 baseline + 29 new)'
  ...
  notes            = 'v4.3.0C expansion seed candidates with v4.3.0C1 content hotfix applied (7s6h false-gutshot fix; 9d6d wait-prose fix; metadata 53/29 fix). ALL auditStatus=planning_only and reviewStatus=v4.3.0C_expansion_candidate; promoted to production via tools/migrate-module4-v4.3.0C.ps1 with two-phase staged approval (review_pending then approved); 2 scenarios re-synced via tools/hotfix-module4-v4.3.0C1.ps1.'
  scenarios        = $scenarios
})
