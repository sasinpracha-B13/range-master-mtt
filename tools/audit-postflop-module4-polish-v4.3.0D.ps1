# ============================================================
# tools/audit-postflop-module4-polish-v4.3.0D.ps1
# v4.3.0D M4 Coverage Polish Seed Auditor
#
# Validates docs/specs/postflop-v4.3.0D-module4-polish-seeds.json
# against M4 schema rules + polish-specific guards.
#
# Mirrors tools/audit-postflop-module4-expansion-v4.3.0C.ps1 with
# polish-aware adjustments:
#   - id pattern ends with _v430D
#   - reviewStatus = v4.3.0D_polish_candidate
#   - cross-corpus duplicate-ID check vs production AND vs v4.3.0C expansion
...
# ---------- Load files ----------
foreach ($p in @($polishPath, $expansionPath, $productionPath)) {
  if (-not (Test-Path $p)) {
...
$polish = $null; $expansion = $null; $prod = $null
...
# ---------- Approved enums (mirror v4.3.0C expansion auditor) ----------
...
$idRegex   = '^pf_btn_v_bb_srp_100bb_turn_[a-zA-Z0-9]+_[0-9a-zA-Z]+_m4_(action|reason)_[A-Za-z0-9_]+_v430D$'

# ---------- TOP-level checks ----------
if ($polish.moduleId -ne 'pf_turn_barrel_oop_def') { Add-Hard 'CR.R02' 'TOP' "moduleId expected pf_turn_barrel_oop_def, got '$($polish.moduleId)'" }
if ($polish.schemaVersion -ne '1.2.0') { Add-Hard 'CR.R02' 'TOP' "schemaVersion expected '1.2.0'" }
if (-not ([string]$polish.version).StartsWith('v4.3.0D')) { Add-Hard 'CR.R02' 'TOP' "version expected to start with v4.3.0D, got '$($polish.version)'" }
if ($polish.status -ne 'planning_only') { Add-Hard 'CR.R02' 'TOP' "status expected 'planning_only', got '$($polish.status)'" }
if (-not ($polish.scenarios -is [System.Array])) { Add-Hard 'CR.R02' 'TOP' "scenarios is not an array" }
...
# Same semantics as v4.3.0C expansion auditor: only fires if a polish ID is
# already in production AND a synced field disagrees.
$prodById = @{}
...
foreach ($polishId in $polishIds) {
  if (-not ($prodIds -contains $polishId)) { continue }
  $ps = $prodById[$polishId]
  $polishS = $polishScenarios | Where-Object { $_.id -eq $polishId } | Select-Object -First 1
  if ($null -eq $ps -or $null -eq $polishS) { continue }
  $diffs = New-Object System.Collections.Generic.List[string]
  if (-not (Compare-ScalarField $ps.recommendedAction $polishS.recommendedAction)) { $diffs.Add('recommendedAction') | Out-Null }
  if (-not (Compare-ScalarField $ps.actionReason      $polishS.actionReason))      { $diffs.Add('actionReason') | Out-Null }
  if (-not (Compare-ScalarField $ps.drawCategory      $polishS.drawCategory))      { $diffs.Add('drawCategory') | Out-Null }
  if (-not (Compare-ScalarField $ps.handClass         $polishS.handClass))         { $diffs.Add('handClass') | Out-Null }
  if (-not (Compare-ScalarField $ps.heroHandRole      $polishS.heroHandRole))      { $diffs.Add('heroHandRole') | Out-Null }
  if (-not (Compare-ScalarField $ps.showdownValue     $polishS.showdownValue))     { $diffs.Add('showdownValue') | Out-Null }
  if (-not (Compare-ScalarField $ps.blockerNote       $polishS.blockerNote))       { $diffs.Add('blockerNote') | Out-Null }
  if ($ps.explanation -and $polishS.explanation) {
    foreach ($k in 'short','turnLogic','rangeContext','handLogic','sizingLogic','commonMistake','takeaway') {
      if (-not (Compare-ScalarField $ps.explanation.$k $polishS.explanation.$k)) { $diffs.Add("explanation.$k") | Out-Null }
    }
  } elseif ($null -ne $ps.explanation -or $null -ne $polishS.explanation) {
    $diffs.Add('explanation') | Out-Null
  }
  if (-not (Compare-ArrayField $ps.conceptTags $polishS.conceptTags)) { $diffs.Add('conceptTags') | Out-Null }
  if ($diffs.Count -gt 0) {
    Add-Hard 'CR.R03' $polishId ("polish source / production drift on fields: " + ($diffs -join ','))
  }
}

# Within-polish duplicate ID check
$dupPolishIds = $polishIds | Group-Object | Where-Object { $_.Count -gt 1 }
foreach ($d in $dupPolishIds) {
  Add-Hard 'CR.R04' 'TOP' "duplicate id within polish '$($d.Name)' x$($d.Count)"
}

# Cross-corpus duplicate vs v4.3.0C expansion (HARD: polish IDs must not collide with expansion IDs)
foreach ($polishId in $polishIds) {
  if ($expansionIds -contains $polishId) {
    Add-Hard 'CR.R05' $polishId "polish ID collides with v4.3.0C expansion ID"
  }
}
...
foreach ($s in $polishScenarios) {
  $sid = if ($s.id) { $s.id } else { '<no id>' }

  # M4.R09 -- id naming convention (v4.3.0D variant)
  if ($s.id -and ($s.id -notmatch $idRegex)) {
    Add-Hard 'M4.R09' $sid "id does not match expected pattern (must end with _v430D)"
  }
  ...
  if ($s.reviewStatus -ne 'v4.3.0D_polish_candidate') { Add-Hard 'M4.R13' $sid "reviewStatus expected 'v4.3.0D_polish_candidate', got '$($s.reviewStatus)'" }
  ...
  if (-not $s.uniquenessNote) {
    ...
    Add-Hard 'M4.R14' $sid "uniquenessNote too short ($($s.uniquenessNote.Length) chars; polish threshold 30)"
  }
  ...
  if (-not $s.answer) { Add-Hard 'M4.R39' $sid 'missing answer' } else {
    if ($qtype -eq 'action_choice') {
      ...
      # (mirrors production auditor R04 strictness; catches action-id leakage into reason_choice answers).
      if ($s.question -and $s.question.choices) {
        ...
        # answer.critical for reason_choice should be empty by convention (close diagnostic mistakes are not severe punts).
...
Write-Host " M4 POLISH SEED AUDIT  --  v4.3.0D" -ForegroundColor Cyan
Write-Host "================================================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host "Polish seed audit summary:" -ForegroundColor Cyan
Write-Host ("  scenarios   = {0}" -f $polishScenarios.Count)
...
Write-Host "--- Distribution ($($polishScenarios.Count) polish scenarios) ---" -ForegroundColor Cyan
Write-Host ""
Write-Host "By turnCategory:"
$polishScenarios | ForEach-Object { $_.board.turnCategory } | Group-Object | Sort-Object Name | ForEach-Object { Write-Host ("  {0,-22} {1}" -f $_.Name, $_.Count) }
Write-Host ""
Write-Host "By qtype:"
$polishScenarios | ForEach-Object { $_.question.qtype } | Group-Object | Sort-Object Name | ForEach-Object { Write-Host ("  {0,-22} {1}" -f $_.Name, $_.Count) }
Write-Host ""
Write-Host "By recommendedAction:"
$polishScenarios | ForEach-Object { $_.recommendedAction } | Group-Object | Sort-Object Name | ForEach-Object { Write-Host ("  {0,-22} {1}" -f $_.Name, $_.Count) }
Write-Host ""
Write-Host "By actionReason:"
$polishScenarios | ForEach-Object { $_.actionReason } | Group-Object | Sort-Object Name | ForEach-Object { Write-Host ("  {0,-30} {1}" -f $_.Name, $_.Count) }
Write-Host ""
Write-Host "By heroHandRole:"
$polishScenarios | ForEach-Object { $_.heroHandRole } | Group-Object | Sort-Object Name | ForEach-Object { Write-Host ("  {0,-22} {1}" -f $_.Name, $_.Count) }
Write-Host ""
Write-Host "By drawCategory:"
$polishScenarios | ForEach-Object { $_.drawCategory } | Group-Object | Sort-Object Name | ForEach-Object { Write-Host ("  {0,-22} {1}" -f $_.Name, $_.Count) }
Write-Host ""
Write-Host "By difficulty:"
$polishScenarios | ForEach-Object { $_.difficultyHint } | Group-Object | Sort-Object Name | ForEach-Object { Write-Host ("  {0,-22} {1}" -f $_.Name, $_.Count) }
Write-Host ""
Write-Host "By sourceConfidence:"
$polishScenarios | ForEach-Object { $_.sourceConfidence } | Group-Object | Sort-Object Name | ForEach-Object { Write-Host ("  {0,-22} {1}" -f $_.Name, $_.Count) }
Write-Host ""
$critWith = (@($polishScenarios | Where-Object { @($_.answer.critical).Count -gt 0 })).Count
$critPct = if ($polishScenarios.Count -gt 0) { ($critWith / $polishScenarios.Count) * 100 } else { 0 }
Write-Host ("Critical-flag scenarios (polish only):  {0} of {1} ({2:N1}%)" -f $critWith, $polishScenarios.Count, $critPct)
$critByValue = @{}
foreach ($s in $polishScenarios) { foreach ($c in @($s.answer.critical)) { if (-not $critByValue.ContainsKey($c)) { $critByValue[$c]=0 }; $critByValue[$c]++ } }
foreach ($k in $critByValue.Keys | Sort-Object) { Write-Host ("  critical={0,-22} {1}" -f $k, $critByValue[$k]) }
Write-Host ""
